...
Jacobian by centered finite differences. This implementation retains the
printed matrix.

## Historical run data

The following data from the original run were not recovered when this
...
The neural switching set `{W<kappa}` extends outside the checked local set
`B_kappa` at 29 sampled points.
...
Lyapunov function may differ from a combination of `W` and `V_l`. The figures
show:

1. `W(x;theta)` with `V_l=kappa/2`, `V_l=kappa`, and `W=kappa` contours;
2. `DW F` for `W` under the printed switching rule.

These figures use the reference run described above.
