...
def _blend_choice(
    ...
    choose_w = np.zeros_like(x1, dtype=bool)
    choose_w[inside] = True
    ...
    if np.any(mid):
        if mode == "w":
            ...
            dist_in = np.maximum(
                np.maximum(xi_min - x1[mid], x1[mid] - xi_max),
                np.maximum(yi_min - x2[mid], x2[mid] - yi_max),
            )
            dist_out = np.minimum.reduce(
                [
                    x1[mid] - x1_min,
                    x1_max - x1[mid],
                    x2[mid] - x2_min,
            ...
            choose_w[mid] = dist_in <= dist_out
        else:
            raise ValueError(f"Unknown blend mode: {mode}")

    choose_w[outside] = False
    return choose_w
...
def _blend_v(
    ...
    choose_w = _blend_choice(x1_tilde, x2_tilde, w_box, x_box, mode)
    V = Vfull.copy()
    dV = dVfull.copy()
    ...
    x1_min, x1_max, x2_min, x2_max = w_box
    xi_min, xi_max, yi_min, yi_max = x_box
    inside = (x1_tilde >= xi_min) & (x1_tilde <= xi_max) & (x2_tilde >= yi_min) & (x2_tilde <= yi_max)
    outside = (x1_tilde < x1_min) | (x1_tilde > x1_max) | (x2_tilde < x2_min) | (x2_tilde > x2_max)
    mid = (~inside) & (~outside)

    use_w = choose_w.copy()
    use_w[mid & choose_w & (~ok_w) & ok_full] = False
    use_w[mid & (~choose_w) & (~ok_full) & ok_w] = True
    use_w[inside] = True
    use_w[outside] = False
